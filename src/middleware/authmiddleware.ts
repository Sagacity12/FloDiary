...
export const authMiddleware = async (
  ...
  try {
    ...
    if (!data.id)
      throw new createHttpError.Unauthorized("Unauthorized: Invalid token");
    const isBlacklisted = await isTokenBlacklisted(token);
    if (isBlacklisted) {
      return next(
        createHttpError.Unauthorized("Unauthorized: Token is blacklisted")
      );
    }
    req.user = data;
    req.token = token;
...
export const authorizeRoles = (allowedRoles: string[]) => {
  return (req: Request, res: Response, next: NextFunction) => {
    ...
    logger.info(
      `User with role ${userRole} is authorized to access this resource`
    );
    next();
  };
};
